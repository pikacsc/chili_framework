...
void DrawPoo(const POO& _poo, Graphics& gfx)
{
	int x = _poo.x;
	int y = _poo.y;
...
void DrawGameOver(const int& x, const int& y, Graphics& gfx)
{
...
void DrawDude(const DUDE& _dude, Graphics& gfx)
{
	int x = _dude.x;
	int y = _dude.y;
...
void DrawWall(const int & _width, const int& _height, Graphics& _gfx)
{
	// 600, 400

	int leftGap = 800 -_width;
	int topGap = 600 - _height;
	int rightGap = 800 - leftGap;
	int BottomGap = 600 - topGap;
	
	for (int x = 0; x < 800; x++)
	{
		for (int y = 0; y < 600; y++)
		{
			if (x > leftGap && x < rightGap && y > topGap && y < BottomGap)
				continue;
			_gfx.PutPixel(x, y, Colors::Cyan);
		}
	}
